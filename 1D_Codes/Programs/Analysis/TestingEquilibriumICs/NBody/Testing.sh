...
parent="Analysis/TestingEquilibriumICs/NBody"
...
ICs choice : %i
Boson std: No Bosons
Particle std = %f \n \n' "2" "$percent" "$num_p" "$fixed_phi" "$bc_choice" "$sim" "$IC_choice" "0.1"

        printf '%s\n%f\n%f\n%i\n%s\n%i\n%i\n%i\n%f' "$parent" "2" "$percent" "$num_p" "$fixed_phi" "$bc_choice" "$sim" "$IC_choice" "0.1" | python3 -u 1D_Codes/Programs/ProgramV2.py 
    done
done
