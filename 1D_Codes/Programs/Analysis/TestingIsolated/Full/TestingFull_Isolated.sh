...
sim choice : %i
Boson std = %f
Particle std = %f \n \n' "2" "$percent" "$v_rms" "$z_rms" "$lambda" "$num_p" "n" "$bc_choice" "$sim" "0.1" "0.1"

    printf '%s\n%f\n%f\n%f\n%f\n%f\n%i\n%s\n%i\n%i\n%f\n%f' "$parent"  "2" "$percent" "$v_rms" "$z_rms" "$lambda" "$num_p" "n" "$bc_choice" "$sim" "0.1" "0.1" | python3 -u 1D_Codes/Programs/ProgramV2.py
done
